...
// This is sample storages_chaincode
...
// Transportation describes chaincode's structure
type Transportation struct {
	Hash                    string   `json:"Hash"`
	OrderTransportationHash string   `json:"OrderTransportationHash"`
	DeliveryVoucherHashes   []string `json:"DeliveryVoucherHashes"`
...
func (t *SimpleChaincode) create(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if !lib.Validate("orders_chaincode", data.OrderTransportationHash, stub) {
		errStr := fmt.Sprintf("Failed to validate %s on %s", "orders_chaincode", data.OrderTransportationHash)
		return shim.Error(errStr)
	}

	if !lib.Validate("actions_chaincode", data.ActionHash, stub) {
		errStr := fmt.Sprintf("Failed to validate %s on %s", "actions_chaincode", data.ActionHash)
		return shim.Error(errStr)
	}

	for i := 0; i < len(data.DeliveryVoucherHashes); i++ {
		if !lib.Validate("delivery_voucher_chaincode", data.DeliveryVoucherHashes[i], stub) {
			errStr := fmt.Sprintf("Failed to validate %s on %s", "delivery_voucher_chaincode", data.DeliveryVoucherHashes)
			return shim.Error(errStr)
		}
	}
