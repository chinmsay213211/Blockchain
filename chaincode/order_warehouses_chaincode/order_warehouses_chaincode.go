...
// OrderWareHouse describes chaincode's state
type OrderWareHouse struct {
	Hash              string   `json:"Hash"`
	OwnerCompanyHash  string   `json:"OwnerCompanyHash"`
	OwnerUserHashes   []string `json:"OwnerUserHashes"`
...
func (t *SimpleChaincode) create(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if !lib.Validate("companies_chaincode", data.OwnerCompanyHash, stub) {
		errStr := fmt.Sprintf("Failed to validate %s on %s", "companies_chaincode", data.OwnerCompanyHash)
