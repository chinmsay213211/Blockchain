...
// Lot describes chaincode's state
type Lot struct {
	Hash              string `json:"Hash"`
	ProductHash       string `json:"ProductHash"`
	ParentLotHash     string `json:"ParentLotHash"`
	OwnerCompanyHash  string `json:"OwnerCompanyHash"`
	OwnerUserHashes   []string `json:"OwnnerUserHash,omitempty"`
	ManagerUserHashes []string `json:"ManagerUserHash,omitempty"`
...
func (t *SimpleChaincode) create(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if !lib.Validate("products_chaincode", data.ProductHash, stub) {
		errStr := fmt.Sprintf("Failed to validate %s on %s", "products_chaincode", data.ProductHash)
	...
	if len(data.ParentLotHash) > 0 && !lib.Validate("lots_chaincode", data.ParentLotHash, stub) {
		errStr := fmt.Sprintf("Failed to validate %s on %s", "lots_chaincode", data.ParentLotHash)
		return shim.Error(errStr)
	}
