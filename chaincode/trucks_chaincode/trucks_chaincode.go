...
// Truck describes chaincode's structure
type Truck struct {
	Hash           string `json:"Hash"`
	TruckMakesHash string `json:"TruckMakesHash"`
	TruckModelHash string `json:"TruckModelHash"`
...
func (t *SimpleChaincode) create(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if !lib.Validate("truck_makes_chaincode", data.TruckMakesHash, stub) {
		errStr := fmt.Sprintf("Failed to validate %s on %s", "truck_makes_chaincode", data.TruckMakesHash)
		return shim.Error(errStr)
	}

	if !lib.Validate("truck_models_chaincode", data.TruckModelHash, stub) {
		errStr := fmt.Sprintf("Failed to validate %s on %s", "truck_models_chaincode", data.TruckModelHash)
		return shim.Error(errStr)
	}
