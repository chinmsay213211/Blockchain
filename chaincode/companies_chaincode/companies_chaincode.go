...
// Company describes chaincode's structure
type Company struct {
	Hash                string   `json:"Hash"`
	Name                string   `json:"Name"`
	//ClientOfCompanyHash string   `json:"ClientOfCompanyHash"`
...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if function == "assignUser" {
		return t.assignUser(stub, args)
	}
...
func (t *SimpleChaincode) assignUser(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	var err error
	...
	dataAsBytes, err := stub.GetState(hash)

	if err != nil {
		str := fmt.Sprintf("Failed to get: %s", hash)
		fmt.Println(str)
		return shim.Error(str)
	} else if dataAsBytes == nil {
		str := fmt.Sprintf("Record does not exist: %s", hash)
	...
	unmarshalledData := Company{}
	err = json.Unmarshal(dataAsBytes, &unmarshalledData)

	if err != nil {
		str := fmt.Sprintf("Can not unmarshal %+v", err.Error())
		fmt.Println(str)
		return shim.Error(str)
	}


	if !lib.Validate("users_chaincode", userHash, stub) {
		errStr := fmt.Sprintf("Failed to validate %s on %s", "users_chaincode", userHash)
		return shim.Error(errStr)
	}

	unmarshalledData.ManagerUserHashes = append(unmarshalledData.ManagerUserHashes, userHash)

	dataJSONasBytes, err := json.Marshal(unmarshalledData)
	...
	err = stub.PutState(hash, dataJSONasBytes)
	...
	fmt.Println("Sucessfully assigned")
	return shim.Success(nil)
}
