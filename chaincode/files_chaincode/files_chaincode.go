...
// File describes chaincode's structure
type FileMain struct {
	Hash              string                `json:"Hash"`
	File              string                `json:"Name"`
	UserHashes        []string              `json:"UserHashes"`
}
...
func (t *SimpleChaincode) create(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	for i := 0; i < len(data.UserHashes); i++ {
		if !lib.Validate("users_chaincode", data.UserHashes[i], stub) {
			errStr := fmt.Sprintf("Failed to validate %s on %s", "users_chaincode", data.UserHashes[i])
