...
// PickList describes chaincode's state
type PickList struct {
	Hash                     string   `json:"Hash"`
	TransportationHash       string   `json:"TransportationHash"`
	OrderTransportationHash  string   `json:"OrderTransportationHash"`
	CommodityHash            string   `json:"CommodityHash"`
	LotsHashes               []string   `json:"LotsHashes"`
	ProductHashes            []string   `json:"ProductHashes"`
	Quantity                 int      `json:"Quantity"`
	StorageHashes            []string `json:"StorageHash"`
	CompanyHash              string   `json:"CompanyHash"`
	ManagerUserHashes        []string   `json:"ManagerUserHashes"`
	Status                   string   `json:"Status"`
	ForkLiftOperatorUserHash string   `json:"ForkLiftOperatorUserHash"`
...
func (t *SimpleChaincode) create(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if !lib.Validate("order_transportations_chaincode", data.OrderTransportationHash, stub) {
		errStr := fmt.Sprintf("Failed to validate %s on %s", "order_transportations_chaincode", data.OrderTransportationHash)
		return shim.Error(errStr)
	}

	for i := 0; i < len(data.LotsHashes); i++ {
		if !lib.Validate("lots_chaincode", data.LotsHashes[i], stub) {
			errStr := fmt.Sprintf("Failed to validate %s on %s", "lot_chaincode", data.LotsHashes[i])
			return shim.Error(errStr)
		}
	}

	for i := 0; i < len(data.ProductHashes); i++ {
		if !lib.Validate("products_chaincode", data.ProductHashes[i], stub) {
			errStr := fmt.Sprintf("Failed to validate %s on %s", "products_chaincode", data.ProductHashes[i])
			return shim.Error(errStr)
		}
	}
	...
	if !lib.Validate("warehouses_chaincode", data.ForkLiftOperatorUserHash, stub) {
		errStr := fmt.Sprintf("Failed to validate %s on %s", "warehouses_chaincode", data.ForkLiftOperatorUserHash)
		return shim.Error(errStr)
	}

	if !lib.Validate("commodities_chaincode", data.CommodityHash, stub) {
		errStr := fmt.Sprintf("Failed to validate %s on %s", "warehouses_chaincode", data.CommodityHash)
		return shim.Error(errStr)
	}
