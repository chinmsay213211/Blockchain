...
// OrderTransportation describes chaincode's state
type OrderTransportation struct {
	Hash                      string   `json:"Hash"`
	RequestFromCompanyHash    string   `json:"RequestFromCompanyHash"`
	RequestFromClientUserHash string   `json:"RequestFromClientUserHash"`
	DeliverFromCompanyHash    string   `json:"DeliverFromCompanyHash"`
	DeliverToCompanyHash      string   `json:"DeliverToCompanyHash"`
	ManagerUserHashes         []string `json:"ManagerUserHashes,omitempty"`
	DriverUserHashes          []string `json:"DriverUserHashes,omitempty"`
	ClientUserHashes          []string `json:"ClientUserHashes,omitempty"`
	TruckHashes               []string `json:"TruckHashes,omitempty"`
	PreOrderHash              string   `json:"PreOrderHash"`
	OrderNumber               string   `json:"OrderNumber"`
	OrderStatus               string   `json:"OrderStatus"`
	LotsHashes                []string `json:"LotsHashes"`
	URLEnabled                bool     `json:"URLEnabled"`
	URLHash                   string   `json:"URLHash"`
	URLHashExpTime            int      `json:"URLHashExpTime"`
	URLCode                   int      `json:"URLCode"`
...
func (t *SimpleChaincode) create(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if !lib.Validate("companies_chaincode", data.RequestFromCompanyHash, stub) {
		errStr := fmt.Sprintf("Failed to validate %s on %s", "companies_chaincode", data.RequestFromCompanyHash)
		return shim.Error(errStr)
	}

	if !lib.Validate("users_chaincode", data.RequestFromClientUserHash, stub) {
		errStr := fmt.Sprintf("Failed to validate %s on %s", "users_chaincode", data.RequestFromClientUserHash)
		return shim.Error(errStr)
	}

	if !lib.Validate("companies_chaincode", data.DeliverFromCompanyHash, stub) {
		errStr := fmt.Sprintf("Failed to validate %s on %s", "companies_chaincode", data.DeliverFromCompanyHash)
		return shim.Error(errStr)
	}

	if !lib.Validate("companies_chaincode", data.DeliverToCompanyHash, stub) {
		errStr := fmt.Sprintf("Failed to validate %s on %s", "companies_chaincode", data.DeliverToCompanyHash)
		return shim.Error(errStr)
	}
	...
	if !lib.Validate("preorders_chaincode", data.PreOrderHash, stub) {
		errStr := fmt.Sprintf("Failed to validate %s on %s", "preorders_chaincode", data.PreOrderHash)
		return shim.Error(errStr)
	}

	for i := 0; i < len(data.LotsHashes); i++ {
		if !lib.Validate("lots_chaincode", data.LotsHashes[i], stub) {
			errStr := fmt.Sprintf("Failed to validate %s on %s", "lots_chaincode", data.LotsHashes[i])
			return shim.Error(errStr)
		}
	}
