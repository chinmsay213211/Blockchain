...
// TransportationLog describes chaincode's structure
type TransportationLog struct {
...
func (t *SimpleChaincode) create(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if !lib.Validate("transportations_chaincode", data.TransportationHash, stub) {
		errStr := fmt.Sprintf("Failed to validate %s on %s", "transportations_chaincode", data.TransportationHash)
